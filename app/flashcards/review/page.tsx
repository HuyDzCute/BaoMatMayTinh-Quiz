...
function CrossSetReviewInner() {
  ...
  const [counts, setCounts] = useState<Record<ReviewFilter, number>>({
    all: 0,
    lapses: 0,
    due: 0,
    new: 0,
        ...
        <main className="flex-1 fc-page">
          ...
          <section className="fc-hero">
            ...
            <p className="fc-hero-sub">
              Hệ thống sẽ gộp mọi thẻ đến hạn từ tất cả bộ (có sẵn + của bạn) vào một phiên ôn duy
              nhất.
            </p>
          </section>
  ...
  if (done) {
    ...
    const breakdown = new Map<
      string,
      { setName: string; setColor: string; total: number; correct: number }
    >();
    ...
    return (
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#0a0f1e" }}>
        ...
        <main className="flex-1 fc-page">
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="fc-set-card-desc">
                        Đúng <strong style={{ color: "#34d399" }}>{b.correct}</strong> / {b.total} ·
                        chính xác {pct}%
                      </p>
                      <div
          ...
          >
            <button type="button" className="fc-btn fc-btn-primary" onClick={rebuildQueue}>
          ...
          <div className="fc-card-stage">
            ...
            {mode === "test" && (
              <div className="fc-test-panel">
                ...
                {testResult ? (
                  <div
                    className={`fc-test-result ${testResult.score >= 0.85 ? "is-good" : testResult.score >= 0.6 ? "is-ok" : "is-bad"}`}
                  >
                ...
                {choiceLocked && (
                  <div className="fc-choice-feedback">
                    {similarity(choicePicked ?? "", currentCard.card.back) > 0.6 ? (
                      ...
                      <>
                        <RotateCcw size={14} /> Đáp án đúng:{" "}
                        <strong>{currentCard.card.back}</strong>
                      </>
                    )}
                  </div>
                )}
            ...
            {showRating && (
              <div className="fc-rating">
                <button
                  type="button"
                  className="fc-rate-btn fc-rate-again"
                ...
                <button
                  type="button"
                  className="fc-rate-btn fc-rate-good"
...
function FilterChips({
  ...
  return (
    <div className="fc-filter-chips" role="group" aria-label="Bộ lọc phiên ôn tập">
      {FILTER_OPTIONS.map((opt) => {
        ...
        return (
          <button
            ...
            title={isDisabled ? `Không có thẻ trong nhóm "${opt.label}"` : `Hiển thị: ${opt.label}`}
          >
