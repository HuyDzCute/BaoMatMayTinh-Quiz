...
export default function QuizPage({ params }: { params: Promise<{ setId: string }> }) {
  ...
  const [sectionSections, setSectionSections] = useState<
    { id: string; passage?: string }[] | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    if (mainSet && mainSet.sections && mainSet.sections.length > 0) {
      ...
      questions = mainSet
        ? [...mainSet.questions].sort(() => Math.random() - 0.5).slice(0, 40)
        : [];
      setSectionPassage("");
      setSectionLabel("");
    }
    ...
    if (saved && saved.questions.length === 0) {
      ...
      if (idsMatch) {
        ...
        setAnswers(migrated);
        setSpeakingAnswers(
          (saved.speakingAnswers as (string | undefined)[] | undefined) ??
  ...
  const doSubmit = useCallback(async () => {
    ...
    try {
      ...
      logger.warn("[quiz] saveResult failed:", err);
    }
    ...
    if (!cloudSaved && isFirebaseConfigured) {
      saveResult(result).catch((err) =>
        logger.warn("[quiz] background saveResult retry failed:", err),
      );
    }
    ...
    return (
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#0a0f1e" }}>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div
            className="w-full max-w-sm rounded-2xl p-8 text-center animate-fade-slide-up"
          ...
          {loadError ? (
            ...
            >
              ...
              <p className="text-sm mb-5" style={{ color: "#94a3b8" }}>
                Có thể kết nối mạng đang chậm hoặc bộ đề chưa sẵn sàng. Vui lòng thử lại hoặc quay
                về trang chủ.
              </p>
            ...
            <div className="flex flex-col items-center gap-3">
              <div
                className="animate-pulse-glow px-6 py-3 rounded-xl text-sm"
                style={{ color: "#60a5fa", fontFamily: "var(--font-jetbrains)" }}
              >
                Đang tải câu hỏi...
              </div>
              <button
    ...
    return (
      ...
      >
        <div className="text-center max-w-md">
          ...
          <p className="text-sm mb-6" style={{ color: "#94a3b8" }}>
            Bộ câu hỏi IELTS đang được cập nhật. Vui lòng quay lại sau.
          </p>
          <button
            onClick={() => {
              playSfx("click");
  ...
  if (preQuizStep === "info") {
            ...
            <div className="ielts-intro-card animate-fade-slide-up">
              <div className="ielts-intro-banner">
                ...
                <p className="ielts-intro-sub">
                  Bạn sắp bắt đầu phần thi {secLabel} theo format chuẩn IDP / British Council.
                </p>
              </div>
              ...
              <div className="ielts-intro-section">
                ...
                <div className="ielts-info-grid">
                  ...
                  <div className="ielts-info-cell">
                    ...
                    <p className="ielts-info-cell-value">{sectionDuration} minutes</p>
                  </div>
              ...
              <div className="ielts-intro-section">
                ...
                {sectionType === "reading" ? (
                  <p className="ielts-intro-instructions">
                    Đọc kỹ đoạn văn bên trái, sau đó chọn đáp án đúng nhất cho mỗi câu hỏi. Có thể
                    tua lại đoạn văn bất kỳ lúc nào.
                  </p>
                ) : (
                  <>
                    <p className="ielts-intro-instructions" style={{ marginBottom: "12px" }}>
                      Đọc kỹ đề bài, lập dàn ý theo gợi ý, sau đó nhấn{" "}
                      <strong style={{ color: "#60a5fa" }}>Start recording</strong> để ghi âm câu
                      trả lời. Bạn có thể re-record nhiều lần.
                    </p>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      {[
                        [<Mic size={13} key="icon" />, "Cho phep truy cap micro khi duoc yeu cau"],
                        [
                          <RotateCcw size={13} key="icon" />,
                          "Co the ghi lai nhieu lan neu chua zuf",
                        ],
                        [
                          <SendHorizontal size={13} key="icon" />,
                          "Nop bai khi da ghi xong tat ca cac cau",
                        ],
                      ].map(([icon, text], idx) => (
                        <div
                          key={idx}
              ...
              <div className="ielts-intro-form">
                <div>
                  ...
                  {preQuizShake && !preQuizName.trim() && (
                    <p className="ielts-intro-error">Please enter your full name to continue.</p>
                  )}
                </div>
    ...
    return (
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#0a0f1e" }}>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div
            className="w-full max-w-md rounded-2xl p-8 animate-fade-slide-up"
            style={{ backgroundColor: "#111827", border: "1px solid #334155" }}
          >
            <div
            ...
            <div className="space-y-3 mb-4">
              <InfoRow
                icon={<Zap size={14} style={{ color: "#3b82f6" }} />}
                label="Bai thi"
                value={quiz.setName}
                iconBg="rgba(59,130,246,0.1)"
              />
              <InfoRow
                icon={<AlertCircle size={14} style={{ color: "#06b6d4" }} />}
                label="So cau hoi"
                value={`${quiz.questions.length} cau`}
                iconBg="rgba(6,182,212,0.1)"
              />
              <InfoRow
                icon={<Clock size={14} style={{ color: "#f59e0b" }} />}
                label="Thoi gian"
                value={`${TOTAL_MINUTES} phut tong · ${PER_QUESTION_SECONDS}s/cau`}
              ...
              <input
                ...
                onBlur={(e) => {
                  e.currentTarget.style.borderColor =
                    preQuizShake && !preQuizName.trim() ? "#ef4444" : "rgba(51,65,85,0.6)";
                }}
              />
              {preQuizShake && !preQuizName.trim() && (
                <p className="text-xs mt-1" style={{ color: "#ef4444" }}>
                  Vui long nhap ten thi sinh
                </p>
              )}
            </div>
  ...
  if (preQuizStep === "name") {
    return (
      <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#0a0f1e" }}>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div
            className="w-full max-w-sm rounded-2xl p-6 animate-fade-slide-up"
            style={{ backgroundColor: "#111827", border: "1px solid #334155" }}
          >
      ...
      <div className="max-w-2xl mx-auto w-full px-4 py-6 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <div
              className="flex items-center gap-1.5 px-1.5 py-1 rounded-lg text-xs font-bold"
              style={{
                backgroundColor: `${perQColor}15`,
                border: `1px solid ${perQColor}40`,
                color: perQColor,
                fontFamily: "var(--font-jetbrains)",
              }}
            >
        ...
        {timerWarning && (
          <div
            ...
            style={{
              backgroundColor:
                timerWarning === "warning-final" ? "rgba(239,68,68,0.12)" : "rgba(245,158,11,0.10)",
...
function InfoRow({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "rgba(15,23,42,0.6)", border: "1px solid rgba(51,65,85,0.3)" }}
    >
      <div
