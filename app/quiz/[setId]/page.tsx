...
export default function QuizPage({ params }: { params: Promise<{ setId: string }> }) {
  ...
  const handleSubmit = useCallback(async () => {
    // IELTS sections render their own in-page confirmation modal — do not double-prompt.
    if (!sectionType) {
      let unanswered = 0;
      if (sectionType === "speaking") {
        unanswered = speakingAnswers.filter((a) => !a).length;
      } else {
        unanswered = answers.filter((a) => a === "-1").length;
      }
      if (unanswered > 0) {
    ...
    await doSubmit();
  }, [answers, speakingAnswers, doSubmit, sectionType]);
  ...
  // ── Quiz active ─────────────────────────────────────────
  // Dispatch: IELTS Reading view (light academic theme)
  if (sectionType === "reading" && quizActive) {
    return (
      <IELTSReadingView
  ...
  if (sectionType === "speaking" && quizActive) {
    return (
      <IELTSSpeakingView
        questions={quiz.questions}
        currentIndex={currentIndex}
