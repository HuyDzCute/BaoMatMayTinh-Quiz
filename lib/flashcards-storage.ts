import { FlashcardBucket, FlashcardProgress, FlashcardSet } from "./types";
...
function lsSet(key: string, val: unknown): boolean {
  ...
  try {
    ...
    if (typeof window !== "undefined") {
      logger.error(`[flashcards-storage] failed to persist ${key}:`, err);
...
export function buildFilteredCrossSetReview(filter: ReviewFilter): CrossSetReview {
  ...
  const perSetMap = new Map<
    string,
    { setId: string; setName: string; setColor: string; due: number }
  >();
  for (const c of cards) {
    const cur = perSetMap.get(c.setId) ?? {
      setId: c.setId,
      setName: c.setName,
      setColor: c.setColor,
      due: 0,
...
export function parseFlashcardImport(raw: string, fileName: string): ImportResult {
  ...
  return {
    ok: true,
    message: `Import thành công ${cards.length} thẻ`,
