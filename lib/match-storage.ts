...
export function recomputeMatchLevel(
  ...
  const totalCompleted = Object.values(records).reduce((sum, r) => sum + (r.attempts ?? 0), 0);
...
export function recordMatchRun(args: {
  ...
  const isNewBest = existing.bestTimeMs === undefined || args.timeMs < existing.bestTimeMs;
  ...
  const { leveledUpTo, state: levelState } = recomputeMatchLevel(all, previousLevel);
