...
export async function saveResult(result: QuizResult): Promise<{ cloud: boolean; local: boolean }> {
  ...
  const uid = currentUser ? currentUser.uid : getOrCreateAnonSessionId();
  ...
  if (isFirebaseConfigured && rtdb) {
    ...
    // 2. Save to global all_history (for cross-device sync)
    //    Also save to user-specific history for faster queries
    try {
      // Global history with uid for filtering
      await rtdbSet(rtdbRef(rtdb, `all_history/${result.id}`), {
        playerName,
        setId: result.setId,
        setName: result.setName,
        score: result.score,
        ...
        date: result.date,
        timestamp: Date.now(),
        uid, // Include uid for filtering
      });
      cloudHistorySaved = true;
    } catch (err) {
      logger.warn("[storage] saveResult all_history failed:", err);
    }

    // User-specific history (for faster queries if needed)
    if (currentUser) {
      try {
        ...
        logger.warn("[storage] saveResult user history failed:", err);
      }
...
// ─────────────────────────────────────────
//   History cloud sync (user-specific via global all_history)
// ─────────────────────────────────────────
export function subscribeHistory(uid: string, cb: (entries: QuizResult[]) => void): () => void {
  ...
  if (isFirebaseConfigured && rtdb) {
    try {
      // Read from all_history and filter by uid (cross-device sync)
      const histRef = rtdbQuery(
        rtdbRef(rtdb, "all_history"),
        rtdbOrderByChild("timestamp"),
        rtdbLimitToLast(500), // Get more to filter
      );
      const unsub = rtdbOnValue(
        histRef,
        (snap) => {
          ...
          // Filter by uid for this user
          const arr: QuizResult[] = Object.entries(data)
            .filter(([, d]) => {
              const v = d as Record<string, unknown>;
              return v.uid === uid;
            })
          ...
          cb(arr.slice(0, 100)); // Return max 100
        },
        (err) => {
          if (!active) return;
