...
export type ChatUser = {
  ...
  isAnonymous?: boolean;
  /** True if this is a local guest (not a Firebase user) */
 ...
 * - Local mode: user là local guest (không phải Firebase user) HOẶC RTDB đã từng fail → dùng localStorage
 * - Cloud mode: user là Firebase user (dù anonymous hay Google) AND RTDB OK → dùng RTDB
 *
...
export function shouldUseLocal(user: ChatUser | null): boolean {
  ...
  if (!user) return true;
  // Local guest (localStorage-based, no Firebase) - ALWAYS use local mode
  if (user.isLocalGuest) return true;
  // Firebase user - try cloud mode, but fallback if RTDB has failed
...
export async function setReaction(
  ...
  if (isFirebaseConfigured && rtdb) {
    try {
      ...
      const snap = await rtdbGet(reactRef);
      if (snap.exists()) {
...
export function useCurrentUser(): ChatUser | null {
  ...
  useEffect(() => {
    ...
    const getName = () => localStorage.getItem("qthtm_player_name") || "Bạn";
    const name = getName();
    const localGuest: ChatUser = { uid: sid, displayName: name, photoURL: null, isAnonymous: true, isLocalGuest: true };
    setUser(localGuest);
      ...
      try {
        const unsub = onAuthStateChanged(auth, (u2) => {
          if (u2) {
            ...
            clearRtdbFailed();
            // Ưu tiên tên từ localStorage (user đã nhập tên) trước Firebase displayName
            const freshName = getName();
            setUser({
              uid: u2.uid,
              displayName: freshName !== "Bạn" ? freshName : (u2.displayName || freshName),
              photoURL: u2.photoURL,
              isAnonymous: u2.isAnonymous,
              isLocalGuest: false, // Firebase user - not local guest
            });
          }
  ...
  }, []); // Run once on mount - name is read from localStorage inside callback
  return user;
}
    ...
    const unsub = onValue(
      ...
      (snap) => {
        ...
        for (const [id, value] of Object.entries(data)) {
          ...
          // Auto-join user to all existing rooms (one-time sync)
          if (!membersMap[user.uid]) {
            rtdbSet(ref(db, `chat_rooms/${id}/members/${user.uid}`), true).catch(() => {});
          }

          // Show ALL rooms (community chat - everyone sees everything)
          list.push({
            id,
            name: String(v.name ?? ""),
...
export function useMessages(roomId: string | null): ChatMessage[] {
  ...
  const me = useCurrentUser(); // Dùng user thật từ Firebase auth
  ...
  useEffect(() => {
    ...
    // LOCAL MODE: user local guest hoặc RTDB chưa sẵn sàng
    if (shouldUseLocal(me)) {
      const tick = () => {
        const store = lsRead();
        const list = store.messages[roomId] ?? [];
        setMsgs(list);
