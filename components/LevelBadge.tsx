...
import { MATCH_LEVELS, getMatchLevel, getNextMatchLevel } from "@/lib/match-storage";
 ...
 */
export default function LevelBadge({ totalCompleted, variant = "compact" }: LevelBadgeProps) {
  ...
  const progressPct = next
    ? Math.min(
        100,
        ((totalCompleted - current.threshold) / (next.threshold - current.threshold)) * 100,
      )
    : 100;
  ...
  if (variant === "compact") {
    return (
      <div
        ...
        title={next ? `${matchesToNext} trận nữa để lên ${next.name}` : "Cấp tối đa!"}
      >
    ...
    >
      ...
      {next ? (
        <div>
          ...
          <p className="text-[11px] mt-1.5 text-right" style={{ color: "#64748b" }}>
