...
import { ArrowLeft, ArrowRight, Send, Mic, MicOff, Square, Clock, RotateCcw } from "lucide-react";
...
const fmtDuration = (s: number) => formatTime(s);
...
function speakingUIReducer(state: SpeakingUIState, action: SpeakingUIAction): SpeakingUIState {
  switch (action.type) {
    case "reset":
      return {
        recState: action.audioUrl ? "recorded" : "idle",
        elapsed: 0,
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          logger.warn("[speaking] save failed:", err);
          dispatchUI({
            type: "save_error",
      ...
      logger.warn("[speaking] getUserMedia failed:", err);
          ...
          <div className="ielts-speaking-record" style={{ marginTop: 20 }}>
            {recState === "unsupported" ? (
              <div className="ielts-speaking-record-warning" role="alert">
                <MicOff size={16} />
                <span>
                  Trình duyệt không hỗ trợ ghi âm. Vui lòng dùng Chrome, Edge hoặc Firefox phiên bản
                  mới nhất để thu âm câu trả lời.
                </span>
              </div>
            ) : recState === "denied" ? (
              <div className="ielts-speaking-record-warning" role="alert">
                <MicOff size={16} />
                <span>
                  Quyền truy cập micro đang bị chặn. Hãy mở cài đặt trình duyệt để cấp quyền micro,
                  sau đó nhấn <em>Ghi lại</em>.
                </span>
              </div>
            ) : saveError ? (
            ...
            <p className="ielts-speaking-record-help">
              ...
              {recState === "recording" && "Đang ghi âm — nói rõ ràng. Nhấn Dừng khi hoàn thành."}
              {recState === "recorded" &&
