...
import { recordMatchRun, formatMatchTime, getMatchLevelState } from "@/lib/match-storage";
    ...
    <div className="w-full">
      ...
      <div className="mb-6" role="status" aria-live="polite" aria-atomic="true">
        <div className="flex items-center justify-between mb-1.5 text-xs">
          ...
          <span style={{ color: "#94a3b8" }}>
            Sai{" "}
            <strong style={{ color: state.mistakes > 0 ? "#f59e0b" : "#f1f5f9" }}>
              {state.mistakes}
            </strong>
          </span>
        </div>
          ...
          {state.leftTiles.map((tile) => {
            return (
              <TileButton
                ...
                isWrong={wrongPair !== null && wrongPair.left === tile.id}
                locked={state.locked}
          ...
          {state.rightTiles.map((tile) => {
            return (
              <TileButton
                ...
                isWrong={wrongPair !== null && wrongPair.right === tile.id}
                locked={state.locked}
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3 text-xs">
        <p style={{ color: "#475569" }}>Nhấn 2 thẻ — 1 thuật ngữ + 1 định nghĩa — để ghép cặp.</p>
        <Link
          href="/match"
          onClick={() => onExit?.()}
